...
import { getMockTestById  } from "@/shared/mock-test";
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3 px-6 pt-8 text-center">
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${stripe}22, ${stripe}44)` }}
          >
            🔒
          </div>
          <h2 className="text-lg font-semibold text-slate-800">This test requires a Pass</h2>
          <p className="text-sm font-light leading-relaxed text-slate-500">
            <span className="font-semibold text-slate-700">{test.title}</span> is a premium test.
            Unlock it with a pass to get full access, instant scoring, and detailed review.
          </p>
        </div>

        {/* Price */}
        {test.price != null && (
          <div className="mx-6 mt-5 rounded-2xl bg-slate-50 py-4 text-center">
            <p className="text-xs text-slate-400">One-time unlock</p>
        ...
        {/* What you get */}
        <div className="mx-6 mt-4 flex flex-col gap-2 rounded-2xl border border-slate-100 bg-white p-4">
          {[
            "Full access to all questions",
            "Instant score & detailed review",
        ...
        <div className="flex flex-col gap-2.5 p-6">
          <button
            className="w-full rounded-xl py-3.5 text-sm font-bold text-white shadow-md transition-all hover:opacity-90"
            style={{ background: "linear-gradient(135deg, #8B5CF6, #7c3aed)" }}
            onClick={() => {
              // TODO: wire up Razorpay / pass purchase flow
              alert("Redirect to purchase flow");
            }}
          >
            🔓 Unlock with Pass
          </button>
          <button
            onClick={onClose}
            className="w-full rounded-xl border border-slate-200 py-3 text-sm font-medium text-slate-500 hover:bg-slate-50"
          >
            Maybe later
          </button>
        </div>
...
export default function TestDetailPage() {
  ...
  const fetchTest = async () => {
    ...
    try {
      ...
      setTest(res.data); // hasAccess is already inside res.data
    } catch {
      setTest(null);
    } finally {
      setLoading(false);
  ...
  const stripe = stripeFor(test.id);
  const isFree = test.accessMode === "FREE" || test.hasAccess;
  ...
  return (
    <>
      {showUnlock && (
        <UnlockModal test={test} onClose={() => setShowUnlock(false)} />
      )}
        ...
        <div className="border-b border-slate-100 bg-white px-5 py-4">
          <div className="mx-auto flex max-w-6xl items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400">
              ...
              <span>/</span>
              <span className="line-clamp-1 font-medium text-slate-700">{test.title}</span>
            </div>
          </div>
        </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {isFree ? (
                          ...
                          <span className="rounded-full bg-violet-50 px-2.5 py-0.5 text-[10px] font-bold text-violet-600 ring-1 ring-violet-200">
                            🔒 PASS REQUIRED
                          </span>
                        )}
                      </div>
                  ...
                  <div className="mt-6 grid grid-cols-2 divide-x divide-slate-100 rounded-xl bg-slate-50 py-3 sm:grid-cols-4">
                    {[
                      ...
                      { icon: "🏆", label: "Total marks", value: String(test.totalMarks), color: "text-violet-600" },
                      {
                        icon: "⚠️",
                        label: "Negative",
                        value: negativeCount > 0 ? `${negativeCount} Qs` : "None",
                        color: negativeCount > 0 ? "text-orange-500" : "text-emerald-500",
                      },
                    ].map((s) => (
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-3">
                      {[
                        { label: "Easy", count: easy, text: "text-emerald-600", bg: "bg-emerald-50" },
                        { label: "Medium", count: medium, text: "text-amber-600", bg: "bg-amber-50" },
                        { label: "Hard", count: hard, text: "text-red-600", bg: "bg-red-50" },
                      ].map((d) => (
                        <div key={d.label} className={`rounded-xl ${d.bg} p-3 text-center`}>
                          <p className={`text-lg font-bold ${d.text}`}>{d.count}</p>
                          <p className="text-xs text-slate-500">{d.label}</p>
                          <p className="text-[10px] text-slate-400">
                            {qCount > 0 ? Math.round((d.count / qCount) * 100) : 0}%
                          </p>
                        </div>
                      ))}
                    </div>
                          ...
                          <div key={name}>
                            <div className="mb-1 flex items-center justify-between">
                              ...
                              <span className="text-xs text-slate-400">
                                {count} questions · {Math.round((count / qCount) * 100)}%
                              </span>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="rounded-xl bg-red-50 p-3">
                        <p className="text-lg font-bold text-red-500">
                          {test.questions[0]?.negativeMarks > 0
                            ? `-${test.questions[0].negativeMarks}`
                            : "0"}
                        </p>
            ...
            <div className="flex flex-col gap-4 lg:sticky lg:top-6 lg:self-start">
              ...
              <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-100">
                {test.thumbnailUrl ? (
                  ...
                  <div
                    className="flex h-40 items-center justify-center text-5xl font-black text-white/30"
                    style={{
                      background: `linear-gradient(135deg, ${stripe}99, ${stripe})`,
                    }}
                  >
                    {initialsFor(test.title)}
                  </div>
                ...
                <div className="p-5">
                  {/* Access info */}
                  {!isFree && test.price != null && (
                    <div className="mb-3 flex items-center justify-between rounded-xl bg-violet-50 px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        <span className="text-base">🔒</span>
                        <span className="text-xs font-semibold text-violet-700">Pass required</span>
                      </div>
                      <span className="text-sm font-bold text-violet-700">₹{test.price}</span>
                    </div>
                  )}
                  ...
                  <p className="mt-2 text-center text-[11px] text-slate-400">
                    {isFree
                      ...
                      : "Unlock with a pass to access this test"}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100">
                ...
                <SidebarRow
                  ...
                  value={`+${test.questions[0]?.marks ?? 1}`}
                ...
                <SidebarRow
                  icon="🎟️"
                  label="Access"
                  value={isFree ? "Free" : "Pass required"}
                  valueClass={isFree ? "text-emerald-600 font-bold" : "text-violet-600 font-bold"}
                />
                <SidebarRow
                  icon="📅"
                  label="Last updated"
