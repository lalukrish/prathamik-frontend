...
function CardSkeleton() {
  return (
    <div className="flex w-[82vw] shrink-0 flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-100 sm:w-[300px]">
...
function TestCard({
  ...
  const qCount = test.questions.length;
  const stripe = stripeFor(test.id);
  const isFree = test.accessMode === "FREE";

  return (
    <div
      className="group flex w-[82vw] shrink-0 cursor-pointer flex-col overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-slate-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl sm:w-[300px]"
      style={{ scrollSnapAlign: "start" }}
      onClick={() => onView(test)}
    >
      {/* Thumbnail */}
      <div className="relative h-40 w-full overflow-hidden">
        ...
        {/* Live/Draft badge — top left */}
        <div className="absolute left-3 top-3">
          {test.isPublished ? (
            <span className="rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold text-emerald-600 shadow-sm backdrop-blur-sm">
              LIVE
            </span>
          ) : (
            <span className="rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold text-slate-400 shadow-sm backdrop-blur-sm">
              DRAFT
            </span>
          )}
        </div>
      ...
      {/* Logo + Title row (below thumbnail) */}
      <div className="flex items-center gap-3 px-4 pt-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {test.description && (
            <p className="truncate text-[11px] text-slate-400">{test.description}</p>
          )}
        </div>
      </div>

      {/* Quick stats */}
      <div className="mt-3 grid grid-cols-3 divide-x divide-slate-100 border-y border-slate-100 bg-slate-50/60 py-2">
        <div className="flex flex-col items-center gap-0.5 px-2">
          <span className="text-xs font-bold text-orange-500">{qCount > 0 ? qCount : "—"}</span>
          <span className="text-[10px] text-slate-400">Qs</span>
        </div>
        <div className="flex flex-col items-center gap-0.5 px-2">
          <span className="text-xs font-bold text-slate-700">{test.durationMinutes}m</span>
          <span className="text-[10px] text-slate-400">Duration</span>
        </div>
        <div className="flex flex-col items-center gap-0.5 px-2">
          <span className="text-xs font-bold text-violet-600">{test.totalMarks}</span>
      ...
      <div className="p-4">
        ...
        >
          {qCount === 0 ? "No questions yet" : isFree ? "Get Started →" : "🔒 Unlock & Start →"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* More coming soon */}
        <div className="flex w-[180px] shrink-0 flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-200 bg-white p-6 text-center">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-orange-50 text-xl text-orange-500">
            →
          </div>
          <p className="text-xs font-semibold text-slate-600">More coming soon</p>
        </div>
      ...
      <div className="mt-2 hidden justify-end gap-2 sm:flex">
        <button
          onClick={() => scrollBy(-1)}
          className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-white text-sm text-slate-500 hover:bg-slate-50"
        >
          ←
        </button>
        <button
...
export default function TestsPage() {
  ...
  const fetchTests = async () => {
    ...
    try {
      ...
      setError("Couldn't load tests. Try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 pb-16">
        {loading ? (
          ...
          <TestScrollRow tests={tests} onView={handleView} />
        )}
      </div>
    </div>
  );
